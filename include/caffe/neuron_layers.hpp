...
template <typename Dtype>
class BinaryLabelDropoutLayer : public NeuronLayer<Dtype> {
 public:
  explicit BinaryLabelDropoutLayer(const LayerParameter& param)
      : NeuronLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_BINARY_LABEL_DROPOUT;
  }
 ...
 protected:
  ...
  inline int ExactNumBottomBlobs() const { return 2; }

  inline int ExactNumTopBlobs() const { return 1; }
